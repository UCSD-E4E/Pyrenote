...
const StickySlider = ({ min = 0, max = 10, hasLabels = false, threshold = 10, stickyPos = [], changeCallback = noop }) => {
  ...
  const handleStick = e => {
    ...
    stickyPos.forEach(pos => {
      if (Math.abs(pos - finalValue) < 20) finalValue = pos;
    });
  ...
  const renderLabels = () => {
    const leftBound = 1.25
    return (
      <div style={{position: "relative", width: "100%", height: "24px", left: "0px", textAlign: "center"}}>
        {
            ...
            return (
              ...
              >
                <div 
                  className="vl" 
                />
                <text style={{position: "relative", left: computeSpacing(index), top: "-5px"}}>{label}</text>
              </div>
